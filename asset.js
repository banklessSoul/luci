...
function updateSVG(connections, minScore) {
	try {
		...
		paths.enter()
			...
			.style("opacity", d => d.correlationScore >= minScore ? 1 : 0); // Set opacity based on minScore
		...
		circles.selectAll("circle")
			...
			.style("opacity", d => d.score >= minScore ? 1 : 0); // Set opacity based on minScore
...
function updateConnectionsAndRender(minScore = 0) {
	...
	const connections = Object.entries(masquradeData).map(([key, value]) => ({
		id: key,
